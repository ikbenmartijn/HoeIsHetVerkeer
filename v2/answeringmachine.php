...
function MaakAntwoord($id,$entities) {
	...
	switch($infoValue) { //MAINSWITCH
		...
		case 1: //MAINSWITCH: enkel event(s) gevonden 
			...
			switch($eventcount) { //EVENTONLYQUERYSWITCH
				case 1: //slechts 1 event
					$eventsonlyquerystring = "SELECT * FROM cachedata2 WHERE type='".$eventarray[0]."';";
					break;
				default: //meer dan 1 events
					$eventsonlyquerystring = "SELECT * FROM cachedata2 WHERE";
			...
			$resulteventsonlyquery = mysql_query($eventsonlyquerystring) or die(mysql_error());
			...
			switch($eventonlyquerycount) {
				...
				default: //meer dan 1 resultaat, beetje meer problemen, samenvatting geven in tweet en meer infopagina meegeven
					...
					switch($eventcount) { //MULTIEVENTONLYQUERYSWITCH
						case 1: //slechts 1 event
							$multieventsonlyquerystring = "SELECT count(id),road FROM cachedata2 WHERE type='".$eventarray[0]."' GROUP BY road ORDER BY COUNT(id),road ASC;";
							break;
						default: //meer dan 1 events
							$multieventsonlyquerystring = "SELECT count(id),road FROM cachedata2 WHERE";
					...
					$resultmultieventsonlyquery = mysql_query($multieventsonlyquerystring) or die(mysql_error());
					...
					switch($multieventonlyquerycount) { //tel aantal wegen
						case 1: //meerdere events, maar ze zijn allemaal op dezelfde weg...
							while($rij = mysql_fetch_array($resultmultieventsonlyquery)) {
								...
								$estring = "http://hoeishetverkeer.ikbenmartijn.be/m.php?only=none&e=";
								$estring = $estring . implode (",", $eventarray);
								$estring = $estring . "&r=".$rij['road'];
								
								//beperk desnoods het antwoord
								$antwoord = substr($antwoord, 0, 94) . $estring;
							}
							break;
						...
						$antwoord = "Er ";
							...
							$estring = "http://hoeishetverkeer.ikbenmartijn.be/m.php?only=e&e=";
							$estring = $estring . implode (",", $eventarray);
		...
		case 2: //MAINSWITCH: enkel weg(en) gevonden
			...
			switch($wegcount) { //ROADONLYQUERYSWITCH
				case 1: //slechts 1 weg
					$roadsonlyquerystring = "SELECT * FROM cachedata2 WHERE road='".$wegarray[0]."';";
					break;
				default: //meer dan 1 weg
					$roadsonlyquerystring = "SELECT * FROM cachedata2 WHERE";
					$forcounter=0;
			...
			$resultroadsonlyquery = mysql_query($roadsonlyquerystring) or die(mysql_error());
				...
				default: //meerdere problemen op mogelijks verschillende wegen (wegcount checken daarvoor)	
					...
					switch($wegcount) { //MULTIROADONLYQUERYSWITCH
						case 1: //slechts 1 weg ingegeven vb @hoeishetverkeer #A10
							$multiroadsonlyquerystring = "SELECT count(id),type FROM cachedata2 WHERE road='".$wegarray[0]."' GROUP BY type ORDER BY COUNT(id),type ASC;";
							break;
						default: //meer dan 1 weg ingegeven vb @hoeishetverkeer #A10 #R8
							$multiroadsonlyquerystring = "SELECT count(id),type FROM cachedata2 WHERE";
					...
					//voer de query uit
					$resultmultiroadsonlyquery = mysql_query($multiroadsonlyquerystring) or die(mysql_error());
					...
					switch($multiroadsonlyquerycount) { //tel aantal wegen
						...
						default: //meerdere soorten events
							...
							while($rij = mysql_fetch_array($resultmultiroadsonlyquery)) {
								...
								if($i==1) {
									if($rij['count(id)']==1) { //1 event van type X
										$quickroadqstring = "SELECT road FROM cachedata2 WHERE ";
										foreach($wegarray as $weg) {
											$qrqscounter++;
											if($qrqscounter==1) {
												$addstring = "road='". $weg ."'";
										...
										$resultquickroadq = mysql_query($quickroadqstring) or die(mysql_error());
										...
										$antwoord = $antwoord . "er zijn " . $rij['count(id)'] . " ". EventSyntax($rij['type'],"meer") ." op ";//WEGEN
										//check of het op meerdere wegen is of enkel op 1
										$quickroadqstring = "SELECT road FROM cachedata2 WHERE type='".$rij['type'] . "' AND (";
										$qrqscounter=0;
										foreach($wegarray as $weg) {
											$qrqscounter++;
											if($qrqscounter==1) {
										...
										if($quickroadcount==1) {
											$quickroadqarray = mysql_fetch_array($resultquickroadq);
											$antwoord = $antwoord . "de ".$quickroadqarray['road'];
										}
										else {
											$antwoord = $antwoord . "die wegen";
										}
									}
								}
								else {
									if($i==$multiroadsonlyquerycount) { //check of het het laatste element van de array is
										...
										if($rij['count(id)']==1) { //1 event van type X
											...
											$antwoord = $antwoord . " " . $rij['count(id)'] . " ". EventSyntax($rij['type'],"meer") ." op ";//WEGEN
											//check of het op meerdere wegen is of enkel op 1
											$quickroadqstring = "SELECT road FROM cachedata2 WHERE type='".$rij['type'] . "' AND (";
											$qrqscounter=0;
											foreach($wegarray as $weg) {
							...
							$estring = "http://hoeishetverkeer.ikbenmartijn.be/m.php?only=r&r=";
							$estring = $estring . implode (",", $wegarray);
		...
		case 3: //MAINSWITCH: combinatie wege(en) en event(s)
			//query bouwen
			$roadsandeventsquerystring = "SELECT * FROM cachedata2 WHERE (";
			...
			$resultroadsandeventsquery = mysql_query($roadsandeventsquerystring) or die(mysql_error());
