<?php
	...
	if(curl_exec($ch) == false) {
		...
		$query = "INSERT INTO cachedates (issueDate) VALUES ('".$obj->issueDate."') ;";
		echo "ISSUEDATA OPSLAAN:<br/><pre>$query</pre><hr/>";
		mysql_query($query);
		
		$query = "SELECT id FROM cachedates ORDER BY id DESC LIMIT 0,1;";
		...
		for($i=0; $i<$AantalEvents; $i++) {
			$query = "INSERT INTO cachedata (id, type, road, roadFrom, roadTo, details, routeIn, routeFrom, routeTo, segment, text) VALUES (".
						$id.", '".
						$obj->events[$i]->type."', '".
		...
		$id = $id-1;
		$query = "DELETE FROM cachedata WHERE id='".$id."';";
		echo "<hr/>OUDE DATA VERWIJDEREN MET ID $id:<pre>$query";
		mysql_query($query);
		$query = "DELETE FROM cachedates WHERE id='".$id."';";
		echo "<br/>$query</pre>";
		mysql_query($query);
	}		
	curl_close($ch);
